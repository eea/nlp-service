...
class RawFARMRanker(FARMRanker):
    ...
    def __init__(
        ...
        super(RawFARMRanker, self).__init__(
            ...
            model_version=model_version, batch_size=batch_size,
            use_gpu=use_gpu, top_k=top_k, num_processes=num_processes,
    ...
    def predict(self,
        ...
        similarity_scores = [
            pred["probability"]
            if pred["label"] == "1" else (1 - pred["probability"])
            for preds in result for pred in preds["predictions"]
        ]
