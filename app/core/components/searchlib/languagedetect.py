...
class LangDetect(BaseComponent):
    ...
    def run(
        ...
        prediction = self.predict(search_term)
    ...
    def predict(self, payload):
        ...
        languages = [meth(text) for text in payload["texts"] if text]
        import pdb

        pdb.set_trace()
        return languages and {"detect_languages": languages} or {}

    def _preprocess(self, search_term, debug=False):
        return {"texts": [search_term], "options": {"debug": debug}}
